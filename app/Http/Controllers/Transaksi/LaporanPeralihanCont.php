...
class LaporanPeralihanCont extends Controller
{
    private $pdf;
    ...
    public function create($id)
    {
        ...
        $laporan = PeralihanNopModel::find($id);

        $no_formulir        = $laporan->no_formulir;
        $nama_wajib_pajak   = $laporan->joinProfilKepada->nama;
        $nik_wajib_pajak    = str_split($laporan->joinProfilKepada->nik);
        $alamat_wajib_pajak = $laporan->joinProfilKepada->alamat;
        $desa_wajib_pajak   = $laporan->joinProfilKepada->joinDesa->nama_desa;
        $kec_wajib_pajak    = $laporan->joinProfilKepada->joinKec->nama_kec;
        $kab_wajib_pajak    = $laporan->joinProfilKepada->joinKab->nama_kab;
        $rt_rw_wajib_pajak      = '';
        $kode_pos_wajib_pajak   = '';

        $nop                        = str_split($laporan->nop);
        $letak_tanah_nop            = $laporan->joinNop->letak;
        $desa_nop                   = $laporan->joinNop->joinDesa->nama_desa;
        $kec_nop                    = $laporan->joinNop->joinKec->nama_kec;
        $kab_nop                    = $laporan->joinNop->joinKab->nama_kab;
        $rtrw_nop                   = $laporan->joinNop->rtrw;
        $luas_tanah_nop             = $laporan->joinNop->getJlLuasTanahAttribute();
        $luas_bangunan_nop          = $laporan->joinNop->getJlLuasBangunanAttribute();
        $njop_tanah_nop             = $laporan->joinNop->getJlNjopTanahAttribute();
        $njop_bangunan_nop          = $laporan->joinNop->getJlNjopBangunanAttribute();
        $njop_x_luas_tanah_nop      = $laporan->joinNop->getJlTanahAttribute();
        $njop_x_luas_bangunan_nop   = $laporan->joinNop->getJlBangunanAttribute();
        // $njop_total                 = number_format($laporan->joinNop->getJlTotalAttribute(), 0, ",", ".");
        $njop_total                 = $laporan->joinNop->getJlTotalAttribute();
        $kode_jenis_perolehan_nop   = str_split($laporan->joinNop->kode_jenis_perolehan);
        $nilai_pasar_nop            = number_format($laporan->joinNop->getJlHakNilaiPasarAttribute(), 0, ",", ".");
        $no_sertifikat_nop          = $laporan->joinNop->no_sertifikat;
        $npop                       = number_format($laporan->npop, 0, ",", ".");
        $npoptkp                    = number_format($laporan->npoptkp, 0, ",", ".");
        $npopkp                     = number_format($laporan->npopkp, 0, ",", ".");
        $bea_terutang               = number_format($laporan->jumlah, 0, ",", ".");
        $jumlah_setor               = number_format($laporan->jumlah_setor, 0, ",", ".");
        $terbilang_setor            = kekata($laporan->jumlah_setor);
        ...
        foreach ($headers as $header) {
            ...
            $pdf->Cell(0, 0, 'No.      ' . $no_formulir, 0, 1, 'R');
            $pdf->SetLineWidth(1);
            $y = $pdf->GetY();
            $pdf->SetXY($x, $y + 3);
            $pdf->Cell(0, 310, '', 1);
            ...
            $pdf->SetXY($x + 95, $pdf->GetY());
            $pdf->Cell(0, 6, '5. RT/RW: ', 0, 0, 'L');
